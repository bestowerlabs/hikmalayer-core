...
pub fn save_state(snapshot: &AppSnapshot) -> std::io::Result<()> {
    ...
    let write_result = (|| -> std::io::Result<()> {
        ...
        // Lock permissions down before any state (which may include key
        // material) touches disk. No-op on non-Unix targets.
        #[cfg(unix)]
    ...
    // (rename preserves the destination inode's permissions if one is
    // replaced on some platforms/configurations).
    #[cfg(unix)]
