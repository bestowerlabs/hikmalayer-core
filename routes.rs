// This is a single-line comment above the code
use axum::{
...
async fn issue_certificate(
    ...
    if !authorize_admin(&headers, &state) {
        return Json(ApiResponse {
            ...
            message: "Unauthorized: a valid admin token is required to issue certificates"
                .to_string(),
        });
    }
...
async fn transfer_tokens(
    ...
    if !authorize_admin(&headers, &state) {
        return Json(ApiResponse {
            status: "error".to_string(),
            message: "Unauthorized: a valid admin token is required to transfer tokens"
                .to_string(),
        });
    }
...
async fn mine_block(State(state): State<AppState>, headers: HeaderMap) -> Json<MiningResponse> {
    if !authorize_admin(&headers, &state) {
        return Json(MiningResponse {
            status: "error".to_string(),
            message: "Unauthorized: a valid admin token is required to trigger mining"
                .to_string(),
            block_index: 0,
...
async fn set_mining_difficulty(
    ...
    if !authorize_admin(&headers, &state) {
        return Json(ApiResponse {
            status: "error".to_string(),
            message: "Unauthorized: a valid admin token is required to change mining difficulty"
                .to_string(),
        });
    }
...
async fn stake_tokens(
    ...
    if !authorize_admin(&headers, &state) {
        return Json(StakeResponse {
            status: "error".to_string(),
            message: "Unauthorized: a valid admin token is required to stake tokens".to_string(),
            total_stake: 0,
        });
    }
    if payload.amount == 0 {
...
async fn withdraw_stake(
    ...
    if !authorize_admin(&headers, &state) {
        return Json(StakeResponse {
            status: "error".to_string(),
            message: "Unauthorized: a valid admin token is required to withdraw stake"
                .to_string(),
            total_stake: 0,
        });
    }
